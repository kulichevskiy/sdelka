...
interface AuthLayoutProps {
  ...
  children: ReactNode
  footer?: ReactNode
  /** Широкие формы (мастер регистрации) на десктопе не должны сжиматься до мобильной колонки */
  width?: 'sm' | 'lg'
}

/** Одна колонка по центру: карточка формы и ссылки под ней */
export function AuthLayout({ title, subtitle, children, footer, width = 'sm' }: AuthLayoutProps) {
  return (
    <div
    ...
    >
      <div
        className={[
          'mx-auto flex w-full flex-1 flex-col justify-center transition-[max-width] duration-200',
      ...
      >
        ...
        <div className="rounded-2xl border border-stone-200 bg-white p-6 shadow-sm sm:p-8 dark:border-stone-800 dark:bg-stone-950">
